...
- This package owns the concrete tenant models (`Tenant`, `TenantDomain`), `Support\VendraTenantResolver` (binds `Misaf\VendraSupport\Contracts\TenantResolver`), `Services\DomainTenantFinder`, the switch tasks (`SwitchAppTask`, `SwitchMailTask`), `Jobs\CacheTenantRoutesJob`, and `TenantServiceProvider`. It is built on Spatie multitenancy.
...
- `Tenant::accessible()` is the request-serving boundary: a tenant must be active, provisioning-ready, and not billing-suspended. Use it for domain resolution and resolver search options; do not offer or resolve inaccessible tenants.
- `ReplaceTenantDomainAction` normalizes and validates a replacement, soft-deletes the previous active domain as history, and creates the new active domain inside the target tenant context.
- The named `GET /caddy/domain-check` endpoint is only for Caddy on-demand TLS authorization. Keep it loopback-only, validate the hostname, and authorize only domains that `DomainTenantFinder` resolves to an accessible tenant.
...
- The `tenants` table carries a nullable `reseller_id` (a billing reseller owned by the **host app**, `Misaf\VendraReseller\Models\Reseller`). It is a plain indexed column with no cross-package foreign key or relation on `Tenant` — the `Reseller → Tenant` relation lives on the app's `Reseller` side only.
...
- Keep `searchOptions` and `DomainTenantFinder` scoped through `Tenant::accessible()`; tenant pickers and host resolution must exclude inactive, billing-suspended, or incompletely provisioned tenants.
